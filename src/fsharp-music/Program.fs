...
let play (wave: Pulse list) =
    ...
    while enum<ALSourceState> (AL.GetSource(alSource, ALGetSourcei.SourceState)) = ALSourceState.Playing do
        Thread.Sleep(100)
...
let freq hz duration =
    ...
    (output, attack, release) |||> List.map3 (fun a b c -> a * b * c)

let f n =
    MathF.Pow(MathF.Pow(2f, 1.0f / 12.0f), n) * pitchStandard
...
let note note' beats (octave: int) =
    let pos =
        float32 <| (int note' - int Note.A) + octave * Enum.GetNames<Note>().Length
...
let combine a b = clipList a b ||> List.map2 (+)
